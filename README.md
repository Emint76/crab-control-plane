...
| Phase 3 execution surface | `operations/harness-phase3/bin/run_phase3_bundle.sh` | canonical execution owner surface; hardened with run-dir invariants, fail-closed behavior, execution target validation, mandatory Phase4 proof for `kb_admission`, canonical reporting, and CI |
| Phase 4 wrapper | `operations/harness-phase4/bin/run_phase4_wrapper.sh` | thin wrapper over Phase 3; generates mandatory `kb_admission` invocation proof; does not own canonical execution outputs; contract: `operations/harness-phase4/PHASE4_WRAPPER_CONTRACT.md` |
...
For `target_kind: kb_admission`, direct Phase3 invocation fails closed unless Phase3 receives and validates the Phase4 invocation claim generated by the wrapper. The gate is documented in `operations/harness-phase4/PHASE4_MANDATORY_INVOCATION_GATE.md`. Non-`kb_admission` Phase3 target kinds are unchanged.
...
A bounded live material-resolution bundle surface is available for green wrapper preflight runs and reviewed outside-Git source declarations:
...
This surface emits repo-local refs-only material-resolution evidence and one wrapper-ready material bundle.
It depends on a green wrapper preflight run plus a reviewed outside-Git declaration, is not the live wrapper, does not load raw secrets, and does not authorize live runtime apply.

A bounded live secret-session bundle surface is available for green material-resolution runs:
...
A bounded live runtime apply surface is available for green wrapper execution-owner runs:
...
This surface re-loads already-approved outside-Git material sources, maps `workspace`, `state`, and `runtime` source classes into the selected outside-Git live roots, and emits canonical wrapper apply evidence plus rollback handoff metadata.
