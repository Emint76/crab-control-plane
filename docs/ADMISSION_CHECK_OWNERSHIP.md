...
For knowledge assets, standalone policy preflight consumes instance-local KB taxonomy configuration through `ADMISSION_KB_TAXONOMY_CONFIG`. The variable is an explicit instance-supplied config path. Absolute paths are accepted, and relative paths resolve against the repository root supplied to the standalone checker. That config is local operator/instance input, not canonical repository taxonomy. Missing config, nonexistent files, non-file paths, invalid JSON, schema-invalid config, unknown selected `knowledge_type`, or selected profile/type mismatch fail closed and must not be reported as real admission readiness. Physical filesystem containment is not enforced by the checker.
