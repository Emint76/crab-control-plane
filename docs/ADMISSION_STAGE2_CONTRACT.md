...
For real knowledge-asset admission readiness, standalone admission policy preflight loads instance-local KB taxonomy configuration from `ADMISSION_KB_TAXONOMY_CONFIG`. The environment variable is an explicit instance-supplied config path. Absolute paths are accepted. Relative paths are resolved against the repository root supplied to the standalone checker. Missing paths, nonexistent files, non-file paths, invalid JSON, and schema-invalid files fail closed. The referenced JSON file must conform to:

```text
operations/admission/schemas/kb_taxonomy_config.v1.schema.json
```

The config is local operator/instance input and must not be committed as real active instance taxonomy. Committed fixtures are non-canonical test inputs only. The config must declare allowed knowledge types and the `knowledge_profile_id -> allowed knowledge_type` mapping. Runtime preflight validates the selected `knowledge_profile_id` and selected `knowledge_type` pair only: the selected type must appear in `allowed_knowledge_types`, the selected profile must have a mapping entry, and the selected type must appear in that selected profile's mapping. Unused future mappings do not block the selected admission. Unknown selected `knowledge_type` or selected profile/type mismatch fails closed for knowledge assets. The canonical repository does not own the active instance taxonomy. Physical filesystem containment is not enforced by the checker.
