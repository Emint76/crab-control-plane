...
Admission Stage 1 and Admission Stage 2 do not themselves admit an asset. Stage 1 defines the universal package contract. Stage 2 defines the universal handoff contract into existing Phase3 target and manifest inputs. Standalone admission policy preflight checks the handoff before Phase execution. The generic Phase2 bundle is a reusable repo/control-plane baseline, not per-asset admission evidence. Canonical admission requires successful Phase3 `kb_admission` evidence.
